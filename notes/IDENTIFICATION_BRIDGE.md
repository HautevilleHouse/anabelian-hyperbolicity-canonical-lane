...
In-paper anchor: `paper/ANABELIAN_HYPERBOLICITY_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving persistence of the declared section, reconstruction, and hyperbolic endpoint package across admissible fundamental-group transport through a multi-lane anabelian-hyperbolic super-architecture`.
...
- endpoint lock: `reconstruction_transfer`.
...
2. the endpoint-transfer lock `reconstruction_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_AHY` remains positive.
...
Payload: verify `reconstruction_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_AHY` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `reconstruction_transfer`
