...
In-paper anchor: `paper/ANABELIAN_HYPERBOLICITY_PREPRINT.md` (`AHY_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving persistence of the declared section, reconstruction, and hyperbolic endpoint package across admissible fundamental-group transport through a multi-lane anabelian-hyperbolic super-architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`AHY_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_AHY`.
...
If near-failure extraction and no-Zeno spacing hold, then `AHY_G3` prevents hidden drift before rigidity is applied.
